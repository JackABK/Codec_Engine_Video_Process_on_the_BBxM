...
static Void scale(SCALE_Handle algo, XDAS_Int8 *in, XDAS_Int8 *out);  //opencv used
...
unsigned char* smain(Int argc, String argv[] )
{
    ...
    */
    ...
    /*===============v4l2 grab frame by Camera======================*/
        ...
        while(count <  300)
        {
            ...
            scale(codec, inBuf , outBuf);
   
            frame->imageData = inBuf;
   ...
   /*===============v4l2 grab frame by Camera But Return main_native.c======================*/
        /*
        int count=0;
        ...
        scale(codec, frame_buffer , frame_buffer);
        return (frame_buffer);
       */
    /*======================================================================================*/
    ...
    while(1)
    {
        ...
        frame_gray=cvCreateImage(size,frame->depth,1);
         
            for (i=0;i<IFRAMESIZE;i++)
            { 
            *(frame_gray->imageData+i) = ( *(frame->imageData+i)*30 + *(frame->imageData+i+1)*59  + *(frame->imageData+i+2)*50 ) / 100;
            }
        //cvCvtColor(frame,frame_gray,CV_8U);
        //cvMerge(frame_gray , NULL , NULL , NULL , frame);

        /* use engine to encode, then decode the data */
        scale(codec, frame , frame_gray);

        /*because the QtGui's format is RGB , and the IplImage->imageData is BGR format , so need to transfer by  it */
        /*cvCvtColor(frame_gray,frame_gray,CV_BGR2RGB);
        qimage =QImage((uchar*)frame_gray->imageData,frame_gray->width,frame_gray->height,QImage::Format_RGB888);
        splash->setPixmap(QPixmap::fromImage(qimage));  
        splash->show();*/

        printf("%d frame is complete \n ",n);
        n++;
...
static Void scale(SCALE_Handle codec, XDAS_Int8 *in, XDAS_Int8 *out)  // opencv used
{
    ...
    //for (n = 0; fread(inBuf, IFRAMESIZE, 1, in) == 1; n++) {
        ...
        //copy the frameImageData to inBuf 

        //memcpy(inBuf,in,IFRAMESIZE);  //v4l2 used

        //memcpy(inBuf,in->imageData,IFRAMESIZE);  //opencv used
        ...
        /*for (i=0;i<IFRAMESIZE;i++)
        {
            *(inBuf+i)    = *(in->imageData + i);
            *(outBuf+i)  = *(out->imageData + i); 
        }*/
        ...
        /* scale the frame */
        status = SCALE_process(codec, in, out, &scaleInArgs,
            &scaleOutArgs);
        ...
        /*opencv used */
         ...
         //memcpy(out->imageData,outBuf,OFRAMESIZE);
